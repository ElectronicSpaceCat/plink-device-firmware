...
//				inloopcount = offset_meas_range_A + offset_meas_range_B; // NOTE: AG - This causes hang
			}
			Status = VL53LX_ClearInterruptAndStartMeasurement(Dev);
			inloopcount++; // NOTE: AG - Fix
		}
...
VL53LX_Error VL53LX_SetVCSELPowerAdjust(VL53LX_DEV Dev, uint8_t VCSEL_power)
{
    VL53LX_Error status = VL53LX_ERROR_NONE;

    if (VCSEL_power > 60 || VCSEL_power < 10) return VL53LX_ERROR_INVALID_PARAMS;

    status = VL53LX_disable_firmware(Dev);
    if(VL53LX_ERROR_NONE == status){
        VL53LX_WaitMs(Dev, 1);
        status = VL53LX_WrByte(Dev, VL53LX_ANA_CONFIG__VCSEL_SELION, VCSEL_power);  // done
    }

    if(VL53LX_ERROR_NONE == status){
        status = VL53LX_enable_firmware(Dev);  // always do this - even if it all failed
    }

    return status;
}

VL53LX_Error VL53LX_GetVCSELPowerAdjust(VL53LX_DEV Dev, uint8_t *VCSEL_power)
{
    VL53LX_Error status = VL53LX_ERROR_NONE;
    ...
    if(VL53LX_ERROR_NONE == status){
        VL53LX_WaitMs(Dev, 1);
        status = VL53LX_RdByte(Dev, VL53LX_ANA_CONFIG__VCSEL_SELION, VCSEL_power);  // done
    }
...
// NOTE: AG - Added this but not yet tested
//VL53LX_Error VL53LX_StartTemperatureUpdate(VL53LX_DEV Dev)
//{
//    VL53LX_Error status = VL53LX_ERROR_NONE;
//    uint8_t isReady = 0, continue_loop = 1;
//    uint16_t i = 0;
//
//    status |= VL53LX_WrByte(Dev, VL53LX_VHV_CONFIG__TIMEOUT_MACROP_LOOP_BOUND, (uint8_t)0x81);
//    status |= VL53LX_WrByte(Dev, VL53LX_VHV_CONFIG__INIT, (uint8_t)0x92);
//    status |= VL53LX_WrByte(Dev, VL53LX_SYSTEM__MODE_START, (uint8_t)0x40);
//
//    VL53LX_ClearInterruptAndStartMeasurement(Dev);
//
//    do{
//            status |= VL53LX_GetMeasurementDataReady(Dev, &isReady);
//            if(isReady == (uint8_t)1) /* Data ready */
//            {
...
//            {
//                    continue_loop = (uint8_t)0;
//                    status = (uint8_t)VL53LX_ERROR_TIME_OUT;
//            }
//            VL53LX_WaitMs(Dev, 1);
...
//
//    status += VL53LX_WrByte(Dev, VL53LX_VHV_CONFIG__TIMEOUT_MACROP_LOOP_BOUND, 0x09);
//    status += VL53LX_WrByte(Dev, VL53LX_VHV_CONFIG__INIT, 0);
//    return status;
//}
